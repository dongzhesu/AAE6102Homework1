...
%   eph          - ephemeris data for satellite position calculation
%
% Outputs:
...
for i = 1:numMeasurements
    % Flag for initialization phase - use more robust settings for the first few epochs
    isInitPhase = (i <= 3);
    
    disp(['Processing epoch ', num2str(i), ' of ', num2str(numMeasurements), '...']);
    if isInitPhase
        disp('*** INITIALIZATION PHASE - Using robust settings ***');
    end
    ...
    % Use increased process noise during initialization
    if isInitPhase
        tempQ = Q * 10;  % Higher process noise during initialization
        disp('Using increased process noise for initialization');
    else
        tempQ = Q;
    end
    
    x_pred = Phi * x;
    P_pred = Phi * P * Phi' + tempQ;
    ...
    % First iteration or initialization phase - normalize innovations
    if isInitPhase
        disp('Initialization phase - adjusting innovations to improve convergence');
        
        % Use a more permissive innovation threshold during initialization
        maxInnovation = 5000; % meters during initialization
    else
        maxInnovation = 100; % meters for regular operation
    end
    
    % Limit very large innovations to improve convergence
    if isInitPhase
        for j = 1:length(innovation)
            if abs(innovation(j)) > maxInnovation
                oldInnovation = innovation(j);
                innovation(j) = sign(innovation(j)) * maxInnovation;
                disp(['  Limiting innovation for PRN ', num2str(availablePRNs(j)), ...
                      ' from ', num2str(oldInnovation), ' to ', num2str(innovation(j))]);
            end
        end
    end
    ...
    % During initialization, force using all measurements
    if isInitPhase
        disp('Initialization phase - using all measurements regardless of innovation size');
        validMeasIdx = true(size(innovation));
    else
    ...
    if sum(validMeasIdx) >= 4
        ...
        % Limit state corrections during initialization for stability
        if isInitPhase
            disp('Initialization phase - limiting state corrections for stability');
            maxPositionCorrection = 1000; % meters
            maxClockCorrection = 5000;   % meters
            
            for j = 1:3 % Position states
                if abs(state_correction(j)) > maxPositionCorrection
                    state_correction(j) = sign(state_correction(j)) * maxPositionCorrection;
                end
            end
            
            if abs(state_correction(4)) > maxClockCorrection
                state_correction(4) = sign(state_correction(4)) * maxClockCorrection;
            end
        end
